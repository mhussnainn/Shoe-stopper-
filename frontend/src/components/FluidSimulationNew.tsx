...
export function FluidSimulation() {
  ...
  useEffect(() => {
    ...
    // ── Light-blue display shader ──────────────────────────────────────
    // Uses three icy blue tones mapped to RGB channels.
    // cA = electric sky blue, cB = deep arctic blue, cC = pale ice blue.
    // Result: silky cyan/blue wisps that glow on dark backgrounds.
    const displayShaderSource = `
     ...
     const vec3 cA = vec3(0.18, 0.32, 0.58);
const vec3 cB = vec3(0.10, 0.24, 0.50);
const vec3 cC = vec3(0.28, 0.42, 0.68);
      void main () {
        ...
        // Map simulation channels to icy blue tones
        vec3 col = raw.r * cA
                 + raw.g * cB
                 + raw.b * cC;

        // Boost luminance so the wisps read clearly
        col = clamp(col * 2.8, 0.0, 1.0);
        ...
        float lum = raw.r * 0.33 + raw.g * 0.33 + raw.b * 0.34;
    ...
    // ── Light-blue colour generation ───────────────────────────────────
    // All generated colours are biased into the blue/cyan family so that
    // the display shader's tint map produces coherent icy blue wisps.
    function generateColor() {
      // Hue range: 185°–225° (cyan-blue to medium blue)
    ...
    function handleFirstTouchStart(e: TouchEvent) {
      ...
      for (let i = 0; i < e.targetTouches.length; i++) {
        updatePointerDownData(pointer, e.targetTouches[i].identifier,
          scaleByPixelRatio(e.targetTouches[i].clientX),
          scaleByPixelRatio(e.targetTouches[i].clientY))
      }
      updateFrame()
    ...
    document.body.addEventListener("mousemove", handleFirstMouseMove as any)
